...
test('No Test Title #3', t => {
  var result = shell.grep(/asdf/g); // too few args
  t.truthy(shell.error());
  t.is(result.code, 2);
});
...
test('No Test Title #4', t => {
  ...
  var result = shell.grep(/asdf/g, '/asdfasdf'); // no such file
  t.truthy(shell.error());
...
test('-l option', t => {
  var result = shell.grep('-l', 'test1', 'resources/file1', 'resources/file2', 'resources/file1.txt');
  t.is(shell.error(), null);
